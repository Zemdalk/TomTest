#!/usr/bin/env bash
# 跑 Tomato：5 个本地模型（不含 deepseek-chat / deepseek-reasoner）。
# 默认 Tomato 使用 genqa（与 ToMBench 保持一致）。

set -euo pipefail

TOMTEST="${TOMTEST:-/home/xujy/TomTest}"
DATASET_ROOT="${DATASET_ROOT:-$TOMTEST/TomDatasets}"
DATASET_NAME="${DATASET_NAME:-Tomato}"
GPU_ID="${GPU_ID:-6}"
PORT="${PORT:-8006}"
VLLM_EXTRA="${VLLM_EXTRA:-}"
EXTRA_RUN_ARGS="${EXTRA_RUN_ARGS:-}"
TOMATO_MODE="${TOMATO_MODE:-genqa}"      # genqa 或 mcqa（mcqa 需 backend=hf，故默认 genqa）
...
for item in "${MODELS[@]}"; do
  ...
  echo "========== [$model_tag] DATASET=${DATASET_NAME} MODE=${TOMATO_MODE} GPU=${GPU_ID} PORT=${PORT} =========="
...
echo "[DONE] Tomato 5 模型完成（已排除 deepseek 系列）。"
